#ExtendedKalmanFilter for Bicycle Model
import numpy as np 

class EKF_BIKE:

    def __init__(self, dt, alpha, sig_r, sig_ph, lr=0.5, L = 1):
        self.dt = dt
        self.alpha1 = alpha[0]
        self.alpha2 = alpha[1]
        self.alpha3 = alpha[2]
    ...
    def EKF_SLAM(self, mu, sigma, u, z, c, detected_flag):
        ...
        phic = u[1]
        number_of_states = 4
        F = np.concatenate( (np.identity(number_of_states) , np.zeros((number_of_states,2*N))) , 1 )
        beta = np.arctan2(self.lr*np.tan(mu[3]),self.L)
        # motion = np.array([[-(vc/wc)*np.sin(mu[2]) + (vc/wc)*np.sin(mu[2]+wc*self.dt)],
        #                    [(vc/wc)*np.cos(mu[2]) - (vc/wc) *np.cos(mu[2]+wc*self.dt)],
        #                    [wc*self.dt]])
        if mu[3] >= np.pi/4 and phic > 0:
            delta_dot = 0
        elif mu[3] <= -np.pi/4 and phic < 0:
            delta_dot = 0
        else:
            delta_dot = phic*self.dt
        motion = np.array([[vc*np.cos(mu[2]+beta) * self.dt],
                            [vc*np.sin(mu[2]+beta) * self.dt],
                            [mu[2] + vc*np.cos(beta)*np.tan(mu[3])/self.L * self.dt],
                            [delta_dot]])
        mu_bar = (mu[:,None] + np.dot( np.transpose(F) , motion)).flatten()
        cov_motion = np.array([[ 0 , 0 , -(vc/wc)*np.cos(mu[2]) + (vc/wc)*np.cos(mu[2] + wc*self.dt) , 0],
                               [ 0 , 0 , -(vc/wc)*np.sin(mu[2]) + (vc/wc)*np.sin(mu[2] + wc*self.dt) , 0],
                               [ 0 , 0 , 0                                                           , 0],
                               [ 0 , 0 , 0                                                           , 0]])
        G = np.identity(number_of_states+2*N) + np.dot( np.transpose(F) , np.dot(cov_motion,F))
        ...
        M = np.zeros((2,2))
        M[0][0] = self.alpha1 * vc**2 + self.alpha4 * phic**2
        M[1][1] = self.alpha2 * vc**2 + self.alpha3 * phic**2
